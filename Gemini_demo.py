...
def render_save_download_section():
    ...
    if has_data or config_items > 0:
                    ...
                    with st.spinner("Creating JSON file..."):
                        ...
                        export_config.update({
                            "export_metadata": {
                                ...
                                "app_version": "Portfolio EVM Suite v1",
        ...
        st.markdown("**[Files] Individual Downloads:**")
        
        col1, col2 = st.columns(2)
        
        with col1:
            # CSV Data download with custom filename
            if st.session_state.data_df is not None and not st.session_state.data_df.empty:
                csv_filename = st.text_input(
                    "CSV Data Filename:",
                    value=f"portfolio_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    key="csv_data_filename"
                )
                csv_data = st.session_state.data_df.to_csv(index=False)
                st.download_button(
                    "[Chart] CSV Data",
                    data=csv_data,
                    file_name=f"{csv_filename.strip().replace('.csv', '')}.csv",
                    mime="text/csv",
                    use_container_width=True,
                    help="Download project data as CSV"
                )
            
            # Batch Results download with custom filename
            if st.session_state.batch_results is not None and not st.session_state.batch_results.empty:
                batch_filename = st.text_input(
                    "Batch Results Filename:",
                    value=f"batch_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    key="batch_results_filename"
                )
                results_csv = st.session_state.batch_results.to_csv(index=False)
                st.download_button(
                    "[Results] Results CSV",
                    data=results_csv,
                    file_name=f"{batch_filename.strip().replace('.csv', '')}.csv",
                    mime="text/csv",
                    use_container_width=True,
                    help="Download EVM calculation results"
                )
        
        with col2:
            # JSON Config download with custom filename
            if config_items > 0:
                config_filename = st.text_input(
                    "Config JSON Filename:",
                    value=f"portfolio_config_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    key="config_json_filename"
                )
                config_json = json.dumps(st.session_state.config_dict, indent=2, default=str)
                st.download_button(
                    "[Config] Config JSON",
                    data=config_json,
                    file_name=f"{config_filename.strip().replace('.json', '')}.json",
                    mime="application/json",
                    use_container_width=True,
                    help="Download configuration settings"
                )
            
            # Clear session data button
            if st.button("Delete Clear Session", 
                        type="secondary", 
                        use_container_width=True,
                        help="Clear all session data (keeps saved files)"):
                if st.session_state.get("confirm_clear_session", False):
                    ...
                    st.warning("Click again to confirm clearing all session data")
    else:
    ...
    try:
                ...
                if valid_count > 0 and "error" not in portfolio_summary:
                    ...
                    if "portfolio_executive_report" in st.session_state:
                        ...
                        if portfolio_report.startswith("Error:"):
                            ...
                            st.download_button("📥 Download Report", portfolio_report, file_name=f"portfolio_executive_report.txt")
        ...
        try:
            ...
            with tab3:
                ...
                Focus on actionable insights for senior stakeholders. {project_context}
                ...
                FORECASTS:
                - Est. at Completion (EAC): {format_currency(results['estimate_at_completion'], controls['currency_symbol'], controls['currency_postfix']) if is_valid_finite_number(results['estimate_at_completion']) else 'N/A'}
                - Expected Finish: {results['forecast_completion']}

                Structure your response with these sections:
                1. EXECUTIVE SUMMARY (3-4 bullets)
                2. CURRENT STATUS (cost and schedule)
                3. FORECAST & OUTLOOK
                ...
                if st.button("🚀 Generate Executive Brief", type="primary"):
                    with st.spinner("Generating executive brief..."):
                        brief_response = safe_llm_request(
                            ...
                            llm_config['ollama_url'], llm_config['temperature'], llm_config['timeout'], prompt
                        )
                ...
                if "executive_brief" in st.session_state:
                    ...
                    if brief.startswith("Error:"):
                        ...
                        st.markdown(brief)
                        st.download_button("📥 Download Brief", brief, file_name=f"executive_brief_{selected_project_id}.txt")
            ...
            with tab4:
                ...
                try:
                    ...
                    pv_func = calculate_pv_scurve if controls['curve_type'] == 's-curve' else calculate_pv_linear
                    pv_values = [pv_func(results['bac'], t, total_months, controls['alpha'], controls['beta']) for t in timeline]
